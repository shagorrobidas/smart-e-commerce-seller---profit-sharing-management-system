...
class DataManager {
  ...
  async apiCall(endpoint, method = "GET", body = null) {
    ...
    const options = {
      method,
      headers: await this.getHeaders(isFormData ? null : "application/json"),
    };
    if (body) {
    ...
    if (!response.ok) {
      ...
      if (responseData) {
        ...
        errorMsg = response.statusText
          ...
          : `API Call Failed (Status ${response.status})`;
      }
  ...
  async addUser(user) {
    return await this.apiCall("/admin/users/create/", "POST", user);
  }
  async getUserById(id) {
    return await this.apiCall(`/admin/users/${id}/`);
  }
  async getStaffUsers() {
  ...
  async addInventoryItem(item) {
    return await this.apiCall("/staff/inventory/", "POST", item);
  }
  async updateInventory(id, updates) {
    return await this.apiCall(`/staff/inventory/${id}/`, "PATCH", updates);
  }
  async getExpenses() {
    return await this.apiCall("/staff/expenses/");
  }
  ...
  async updateOrderStatus(id, status) {
    return await this.apiCall(`/staff/orders/${id}/`, "PATCH", { status });
  }
  ...
  async updateTaskStatus(id, status) {
    const role = JSON.parse(localStorage.getItem("smartEcoUser"))?.role;
    const endpoint =
      role === "admin" ? `/admin/tasks/${id}/` : `/staff/tasks/${id}/`;
  ...
  async getInvestorDashboard() {
    return await this.apiCall("/investor/dashboard/");
  }
  async getInvestorReports() {
    return await this.apiCall("/investor/reports/");
  }
  async getInvestorAgreements() {
    return await this.apiCall("/investor/agreements/");
  }

  // INVESTMENTS
  async getInvestments() {
    return await this.apiCall("/investor/investments/");
  }
  ...
  // SETTINGS
  async getSettings() {
    return await this.apiCall("/admin/settings/");
  }
  async updateSettings(updates) {
    return await this.apiCall("/admin/settings/", "PATCH", updates);
  }
  ...
  async updateProfile(data) {
    return await this.apiCall("/auth/profile/", "PATCH", data);
  }
  async getBusinesses() {
    return await this.apiCall("/auth/businesses/");
  }
  ...
  async resetPassword(email, password) {
    return await this.apiCall("/auth/reset-password/", "POST", {
      email,
      password,
    });
  }
